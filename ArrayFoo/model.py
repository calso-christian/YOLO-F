...
class Output_Activation(tf.keras.layers.Layer):
    def __init__(self, NUM_classes, anchors, GS_alpha, WH_power):
        super().__init__()
...
class Post_Process(tf.keras.layers.Layer):
...
def predict(model, x):
  ...
  y = [Output_Activation(config['NUM_classes'],
                        ...
                        config['PARAMS_WH_power'][level])(y[level]) for level in [0, 1, 2]]
  y = Post_Process(config['NUM_classes'], 15, 0.2, 0.7)(y)
  return y